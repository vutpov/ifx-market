<?php

    require_once('custom-page-header.php')

?>


<div class="container mission">

    <h3>Deposit Methods</h3>
    <div class="horizontal-bar"></div>

    Bank name: ACLEDA Bank<br><br>

    Account name: IFX Market.<br><br>

    Account Number: 1800-02- 256367-1- 7<br><br>

    SWIFT: ACLBKHPP<br><br>

    Or please visit our office for deposit with address:<br>
    No. 19, Street 392 (Corner Street 71)<br>
    Boeung Keng Kang I, Chamkarmon<br>
    Phnom Penh, Kingdom of Cambodia<br>
    Tel: +855 23 957 888<br>
    Email: info@IFX.com.kh<br>


    <br>
    <h3> Withdrawal Method</h3>
    <div class="horizontal-bar"></div>
    Please send withdrawal request to finance@IFX.com.kh for further support or visit our office for withdrawal with address:
    <br><br>
    No. 19, Street 392 (Corner Street 71)<br>
    Boeung Keng Kang I, Chamkarmon<br>
    Phnom Penh, Kingdom of Cambodia<br>
    Tel: +855 23 957 888<br>
    Email: info@IFX.com.kh<br>

</div>



<?php

    require_once('custom-page-footer.php')

?>